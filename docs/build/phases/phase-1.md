...
sheets Items, Variants, Modifiers, Combos; parser (read-excel-file for XLSX per ADR-0013, CSV fallback); validation report
...
- `@rp/domain` `menu-import.ts` (`planMenuImport`, `MENU_IMPORT_COLUMNS`) holds all the rules.
  `csv.ts` gains `parseCsv`.
- Server `src/menu/import/`: `workbook.ts` (read-excel-file, cell text), `zip-limits.ts`
  (unpacked-size guard), `menu-template.ts` and its CLI (`pnpm --filter @rp/server menu:template`),
  and `menu-import.service.ts`.
- Routes, all `MENU_MANAGE`:
  - `GET /api/v1/menu/import/template`;
  - `POST /api/v1/menu/import/check` (dry run);
  - `POST /api/v1/menu/import` (commit and publish).
    Both POST routes answer with a `MenuImportReport` (`ok`, `committed`, `issues`, `summary`,
    `menuVersion`).
- The import only adds, and never changes or archives what exists:
  - existing categories and modifier groups are reused by name;
  - tax groups and stations must already exist and are matched by name, ignoring case;
  - an item already on the menu is an error.
- The commit plans again inside the setup lock and writes through the menu services' `...In(tx)`
  methods, so each entry is audited as in the editor. It adds `MENU_IMPORTED` and publishes, all
  in one transaction.
- Measured: 151 items with variants, modifiers and a combo import in a few seconds (the test
  allows 30 s).
- The console upload screen belongs to the manager UI (P4).
...
P1-13 is split in two.

### P1-13a Report data

As built: `@rp/domain` `reports.ts`, `apps/server/src/reports/` and migration
`20260927020000_report_indexes`.

- `GET /api/v1/reports/{sales,items,payments,shifts,gst,invoice-register}?from=&to=`
  (`REPORTS_VIEW_EXPORT`).
  - The range is in business dates (the register uses invoice dates), at most 366 days.
  - Cashiers (§4.2 OWN) get the shift report for their own shifts and no other report.
- Figures come from invoices as issued, at their current version. Voided invoices count in no
  totals.
- Sales: totals by business date and by hour (restaurant time).
- Items and categories:
  - quantity, gross, discounts, net (taxable) and tax;
  - line tax comes from `lineTaxShares`, which spreads each group's tax over its lines by taxable
    value;
  - a dish is counted once even when equal parts each list it.
- Payments: per mode (with the other-mode name), by the business date they were taken.
- Shifts: expected cash, counted cash and variance, with the staff name.
- GST: `gstSummary` groups taxable value and each component by SAC and combined rate, from one
  cluster per invoice, tax group and taxable value.
- Invoice register: every number per series, financial year and sequence, with status, the void
  reason, the replaced number, and taxable value, tax and total.
- Index on `invoices (restaurant_id, invoice_date)` for the register (RPT-018).

### P1-13b CSV export and order drill-down

- CSV export of each report, stamped with the restaurant, the filters, and who generated it and
  when. Exports are audited (RPT-017).
- Order-level drill-down (RPT-015): who created an order, the source device, who approved it, KOT
  and status times, who modified, cancelled or voided items, and who printed and settled the bill.

As built: `@rp/domain` `csv.ts`, `apps/server/src/reports/report-export.service.ts` and
`order-drill-down.service.ts`.

- `POST /api/v1/reports/exports` `{ report, from, to, format: 'CSV' }` returns
  `{ filename, contentType, content, generatedAt }`.
  - The file starts with the report title, restaurant, From/To (and "Shifts of" for a cashier),
    generated by and generated at (restaurant time), then one or more sections.
  - Amounts are plain rupees; formula-like text is neutralised.
  - Audited as REPORT_EXPORTED. Cashiers may export only their own shift report.
- `GET /api/v1/reports/orders/:orderId` returns the order, creator, device, approvals, items with
  every status time, KOTs, the order history (state changes with actor and device), audited
  actions on the order, its items, bill and invoices (actor and approver), and per invoice who
  issued, printed (first print, now audited as INVOICE_PRINTED), settled or voided it.
